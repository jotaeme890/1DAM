...
public class Ejercicio19 {
    ...
    public static long binarioADecimal(long binario) {
        ...
        int bits = varias.digitos((int)binario);
        
        for(int i = 0; i < bits; i++) {
        decimal += varias.digitoN((int)binario, bits - i - 1) * varias.potencia(2, i);
        }
    ...
    public static long octalABinario(long octal) {
        ...
        for (int i = 0; i < varias.digitos((int)octal); i++) {
        binario = binario * 1000 + decimalABinario(varias.digitoN((int)octal, i));
        }
    ...
    public static long decimalABinario(int decimal) {
        ...
        while (decimal > 1) {
        binario = varias.pegaPorDetras((int)binario, decimal % 2);
        decimal = decimal / 2;
        }
